 ...
 * Fitzpatrick, Eric Foster-Johnson, Daniel Feygin, Scott Daub
 */
...
public abstract class AbstractXmlParser
{
	...
	public void readTree(Vector buf) throws IOException
	{
		...
		// // if (buf != null) buf.addElement (start); [caused duplication,
		// fixed
		// // by SD]
	...
	public ParseEvent read(int type, String namespace, String name) throws IOException
	{
		if (peek(type, namespace, name))
			...
			throw new ParseException("in tag <" + name + ">, unexpected : " + peek(), null, peek().getLineNumber(), -1);
	}

	public boolean peek(int type, String namespace, String name) throws IOException
	{
		// adawy changed this to match tag names case insenstive.
		ParseEvent pe = peek();
		return pe.getType() == type && (namespace == null || namespace.equals(pe.getNamespace()))
				&& (name == null || name.equalsIgnoreCase(pe.getName()));
	}
	...
	public void skip() throws IOException
	{
		...
		{
			int type = peek().type;
			// [Jan 11, 2011 7:10:28 PM] [Amr.ElAdawy] [only processing instruction to be removed]
			//			if (type != Xml.COMMENT && type != Xml.DOCTYPE && type != Xml.PROCESSING_INSTRUCTION && type != Xml.WHITESPACE)
			if (type != Xml.PROCESSING_INSTRUCTION)
				break;
			read();
		}
	}
	...
	public String readText() throws IOException
	{
		...
		while (true)
		{
			...
			case Xml.END_DOCUMENT:
			case Xml.DOCTYPE:
				// adawy commented this to return the text in case of mixed tag i.e. text + tag
