...
public class DataTable extends Table
{
		...
		try
		{
			...
			if (sortedColumn != null)
			{
				...
				else
				{
					...
					sql = new StringBuilder("select * from (").append(sql).append(") a order by ").append(sortKey).append(" ")
							.append(sortedColumn.getSortDirection()).toString();
				}
			}
	...
	protected void renderNewItemPanel(RenderPrinter outputStream) throws Exception
	{
		...
		for (DataPK pk : pks)
		{
			...
			keyname = pk.getName();
			String wcId = new StringBuilder(getId()).append(nameSplitter).append(nameSplitterInsert).append(keyname).toString();
	...
	public void delete(String eventArgs)
	{
		...
		else if (pks.size() > 0)
		{
			try
			{
				String deleteStatement = new StringBuffer("delete from ").append(getTableName()).append(getWhereString(eventArgs)).toString();
	...
	protected String getSqlValue(String key, String value) throws DataException
	{
		...
		{
			// value = String.valueOf(DAO.getSeqNextVal(getDataSource(),
			// pk.getSequence()));
