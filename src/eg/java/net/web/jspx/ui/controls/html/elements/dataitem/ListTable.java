...
public class ListTable extends Table {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ListTable.class);
    ...
    public void dataBind() {
        try {
            ...
            int toIndex = Math.min(itemsList.size(), (index - 1) * size + size);
        ...
        for (DataColumn column : columns) {
            if (column.getType().equalsIgnoreCase("lookup")) {
                String name = column.getParent().getName() + nameSplitter + (isEdit ? nameSplitterUpdate : nameSplitterInsert)
                        + column.getFieldName();
                String val = postedInternalValues.get(name);
    ...
    public List<Object> getSelectedRows() {
        ...
        if (isSelectedAll()) {
            if (isAllSelected())
                return itemsList; // return all the list
            else {
                return (List<Object>) rows; // return the list in the current
                // page
            }
        } else { // return selected indecies
