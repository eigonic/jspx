...
public class AjaxPanel extends GenericWebControl implements IAjaxSubmitter
{
	...
	public void renderChildren(RenderPrinter outputStream) throws Exception
	{
		...
		if (!page.isPostBack && renderRefreshtime)
		{
			...
			String code = new StringBuffer("$jspx(document).ready(function() {setTimeout(\"").append(getAutoRefreshScript()).append("\",")
					.append(getRefreshTime()).append(");});").toString();
				...
				&& ((WebControl) page.getActiveAjaxSubmitter()).getId().equals(getId()))
		{
			...
			else
			{
				...
				// outputStream.write("<script type=\"text/javascript\" language=\"javascript\">");
				...
				// if (!StringUtility.isNullOrEmpty(page.response.get))
				// {
				// outputStream.write("window.parent.");
				// outputStream.write(getOnRender() + ";");
				// }
				//
				// outputStream.write("window.parent.parseScript(document.getElementById('");
				// outputStream.write(getId());
				// outputStream.write("').innerHTML);");
			}
