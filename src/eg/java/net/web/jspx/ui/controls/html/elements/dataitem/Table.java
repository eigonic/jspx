...
public abstract class Table extends HiddenGenericWebControl implements InternalValueHolder, InternalEventsListener, IAjaxSubmitter
{
	...
	protected void renderTableFooter(RenderPrinter outputStream) throws Exception
	{
		...
		// if (getDelete().equalsIgnoreCase("true"))
		// colCount++;
		// if (getEdit().equalsIgnoreCase("true"))
		// colCount++;
	...
	private String getHoverScript()
	{
		...
		String code = new StringBuffer("jQuery( function($) {$('#").append(getId()).append("').tableHover({rowClass:'").append(hoverClass)
				.append("',colClass:''});});").toString();
		return code;
	}
	...
	protected List<Object> reOrganizeRows(List<DataColumn> groupByCols)
	{
		...
		for (int i = 0; i < rows.size(); i++)
		{
			...
			for (DataColumn col : groupByCols)
			{
				...
				groupByExpr += col.getColumnValue().toString() + ",";
			}
			if (headers.add(groupByExpr))
			{
	...
	protected void findSortingColumn(String eventArgs)
	{
		...
		if (index > 0)
		{
			...
			if (columns.size() == 0)
			{
				...
				column.setFieldName(String.valueOf(contText));
	...
	protected DataColumn getSortingColumn()
	{
		...
		// for (DataColumn column : columns)
		// if (!StringUtility.isNullOrEmpty(column.getSortDirection()))
		// return column;
		// return null;
	}
	...
	protected boolean isActionAllowed(String action)
	{
		...
		{
			for (DataColumn column : columns)
				if (column instanceof DataColumnCommand && ((DataColumnCommand) column).getType().equalsIgnoreCase(DataColumnCommand.Edit))
					return true;
			return false;
		}
		...
		{
			for (DataColumn column : columns)
				if (column instanceof DataColumnCommand && ((DataColumnCommand) column).getType().equalsIgnoreCase(DataColumnCommand.Delete))
					return true;
			return false;
		}
	...
	protected void renderTableParts(RenderPrinter outputStream) throws Exception
	{
		...
		outputStream.write(TagFactory.Table);
		outputStream.write(Render.endTag);
		// renderHoverScript(outputStream);
		// ///////
	}
	...
	protected void writeAllDbRowsToFile(ObjectToCSV toCsv, ExportToExcel exportToExcelControl, List<DataColumn> fields, OutputStream out)
			throws Exception
	{
		// int rowsPerLoop = 5000;
		// for(int i = 0; i < exportToExcelControl.getMaxRows(); i=i+rowsPerLoop){
		// List<?> rowsToBeExported = DAO.search(((DataTable) this).getDataSource(), ((DataTable) this).getFinalSql(),
		// i,rowsPerLoop
		// , false, null);
		//
