...
import eg.java.net.web.jspx.engine.util.RequestUtil;
...
import eg.java.net.web.jspx.ui.controls.html.elements.markers.IControlValidate;
import eg.java.net.web.jspx.ui.controls.html.elements.markers.IValidate;
...
public class Page
{
	...
	// protected HashMap<String, Object> requestJspxBeans = new HashMap<String,
	// Object>();
	...
	protected void validatePostbackData() throws ValidationException
	{
		...
		for (IValidate validateItem : validators)
		{
			...
			{
				((IControlValidate) validateItem).validate();
			}
		}
	}
	 ...
	 * 
	 * @param newPage
	 *            the name of the page execution will be transfered to .e.g. // * PageCalss.getClass().getName()
	 */
	...
	public String getPostBackData(String key)
	{
		...
		else
		{
			...
			try
			{
				...
				// if (isAjaxPostBack && !StringUtility.isNullOrEmpty(result))
				// result = StringUtility.decodeUTF8(result);
	...
	public void renderOnloadScripts(RenderPrinter outputStream) throws Exception
	{
		...
		if (!isOnLoadScriptRendered)
		{
			if (onloadScript.length() != 0)
			{
				...
				sb.append("jQuery( function($) {$(document).ready(function (){").append(onloadScript).append("});").append("} );");
	...
	protected void localizeMasterPage()
	{
		try
		{
			...
			else
			{
				...
				((ContentPage) this).setLocale(this.getLocale());
			}
		}
		catch (Exception e)
		{
	 ...
	 * 
	 * @param file
	 *            the file data as a byte [].
	 * @param fileName
	 *            the name of the file that is suggested for saving.
	 */
	 ...
	 * 
	 * @param path
	 *            the path of the jspx portlet file relative to the webroot folder
	 * @return
	 * @throws Exception
	 */
	...
	public void refreshAjaxPanel(String panelId, String eventName, String args)
	{
		...
		StringBuilder s = new StringBuilder().append("postBack('").append(panelId).append("','").append(eventName).append("','" + submitter + "','")
				.append(panelId).append("',false,'").append(args).append("','');");
		response.setHeader(RequestHandler.AjaxPanelRefreshHeader, s.toString());
	}
