...
public class ResourceBundleUtility
{
	...
	private static Object chacheLock = new Object();
	...
	private static String buildBundleName(String name, String locale)
	{
		...
		if (name.charAt(0) != '/')
			name = "/" + name;
		StringBuilder builder = new StringBuilder(name).append(locale);
		String bundleName = builder.toString();
		bundleName = bundleName.replace('.', '/').concat(sufix).toString();
		return bundleName;
	}
