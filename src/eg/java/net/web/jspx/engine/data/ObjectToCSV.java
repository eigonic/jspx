...
public class ObjectToCSV
{
	...
	private Hashtable<String, SimpleDateFormat> dfMap = new Hashtable<String, SimpleDateFormat>();
	...
	private Hashtable<Integer, List<String>> rowCache = new Hashtable<Integer, List<String>>();
			...
			if (o != null)
			{
				...
				for (DataColumn dataColumn : dataColumns)
				{
					...
					else
					{
						...
						//
						// if (dataColumn.getType().equalsIgnoreCase(DataColumn.Date)
						// && (resultObject instanceof Date || resultObject instanceof Timestamp || resultObject instanceof
						// java.sql.Date))
						// {
						// row.add(getDf(dataColumn.getFieldName()).format((Date) resultObject));
						//
						// }
						// else
						// {
						// row.add(resultObject.toString());
						// }
					}
				}
				rows.add(row);
